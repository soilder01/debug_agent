...
def create_final_attribution_verification_job(
    ...
    if follow_up is None:
        raise HTTPException(status_code=404, detail=f"Final attribution follow-up not found: {target_id}")
    actor = _resolved_actor(request.actor)
    _raise_if_usage_budget_blocks_submission()
    follow_up_job = job_service.submit_case_debug(job.case_id, baseline_trials=job.baseline_trials)
    lineage = job_repository.save_strategy_follow_up_job(
...
def create_final_attribution_recovery_job(
    ...
    if follow_up is None:
        raise HTTPException(status_code=404, detail=f"Final attribution recovery follow-up not found: {target_id}")
    actor = _resolved_actor(request.actor)
    _raise_if_usage_budget_blocks_submission()
    follow_up_job = job_service.submit_case_debug(job.case_id, baseline_trials=job.baseline_trials)
    lineage = job_repository.save_strategy_follow_up_job(
...
def create_final_attribution_reinvestigation_job(
    ...
    if follow_up is None:
        raise HTTPException(status_code=404, detail=f"Final attribution reinvestigation follow-up not found: {target_id}")
    actor = _resolved_actor(request.actor)
    _raise_if_usage_budget_blocks_submission()
    follow_up_job = job_service.submit_case_debug(job.case_id, baseline_trials=job.baseline_trials)
    lineage = job_repository.save_strategy_follow_up_job(
...
def list_strategy_follow_up_jobs(job_id: str) -> StrategyFollowUpJobListResponse:
    ...
    return StrategyFollowUpJobListResponse(
        follow_ups=[
            ...
            for follow_up in build_strategy_follow_up_results(job_repository, job_id)
        ]
    )
...
def _build_final_attribution_verification_feedback_summary() -> ObservabilityFinalAttributionVerificationFeedbackSummary:
    outcomes = [
        StrategyFollowUpJobWithOutcome.model_validate(follow_up)
        for source_job_id in {
            follow_up.source_job_id
...
def _final_attribution_follow_up_from_report(report: DebugReport, target_id: str) -> dict[str, str] | None:
    for follow_up in report.follow_up_experiments:
        if follow_up.get("source") == "final_attribution" and follow_up.get("target_id") == target_id:
            return follow_up
    return None


def _final_attribution_recovery_follow_up_from_report(report: DebugReport, target_id: str) -> dict[str, str] | None:
    for follow_up in report.follow_up_experiments:
        if (
...
def _final_attribution_reinvestigation_follow_up_from_report(report: DebugReport, target_id: str) -> dict[str, str] | None:
    for follow_up in report.follow_up_experiments:
        if (
