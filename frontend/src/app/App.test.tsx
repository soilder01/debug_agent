...
describe("App", () => {
  ...
  it("updates recommended action status from a persisted report", async () => {
    ...
    render(<App />);
    await userEvent.click(screen.getByRole("button", { name: "Load debug jobs" }));
    await userEvent.click(await screen.findByRole("button", { name: "Open job job-action-status-1" }));
    await userEvent.click(screen.getByRole("button", { name: "Load persisted report" }));
    await userEvent.click(await screen.findByRole("button", { name: "Accept recommended action 1" }));
  ...
  it("creates a final attribution verification job from a persisted report", async () => {
    ...
    render(<App />);
    await userEvent.click(screen.getByRole("button", { name: "Load debug jobs" }));
    await userEvent.click(await screen.findByRole("button", { name: "Open job job-final-attribution-source" }));
    await userEvent.click(screen.getByRole("button", { name: "Load persisted report" }));
    await userEvent.click(
      await screen.findByRole("button", { name: "Run final attribution follow-up multimodal:conflict:1" })
    );
    ...
    expect(await screen.findByText("Job ID：job-final-verify-1")).toBeInTheDocument();
  });
  ...
  it("creates a final attribution recovery job from a persisted report", async () => {
    ...
    render(<App />);
    await userEvent.click(screen.getByRole("button", { name: "Load debug jobs" }));
    await userEvent.click(await screen.findByRole("button", { name: "Open job job-final-recovery-source" }));
    await userEvent.click(screen.getByRole("button", { name: "Load persisted report" }));
    await userEvent.click(
      await screen.findByRole("button", { name: "Run final attribution recovery multimodal:conflict:1" })
    );
    ...
    expect(await screen.findByText("Job ID：job-final-recovery-1")).toBeInTheDocument();
  });
  ...
  it("shows persisted field count for spreadsheet writeback audits", async () => {
    ...
    expect(await screen.findByText("Writeback audit fields：2")).toBeInTheDocument();
  });
  ...
  it("refreshes writeback audit list and summary after retrying an audit row", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/spreadsheets/writeback/audits?status=failed&limit=50");
    expect(fetchMock).toHaveBeenCalledWith("/api/spreadsheets/writeback/audits/summary");
    expect(await screen.findByText("Writeback audits total：0")).toBeInTheDocument();
  ...
  it("loads more spreadsheet writeback audits using the current status filter", async () => {
    ...
    render(<App />);
    await userEvent.click(screen.getByRole("button", { name: "Load failed writeback audits" }));
    await userEvent.click(await screen.findByRole("button", { name: "Load more writeback audits" }));

    expect(fetchMock).toHaveBeenCalledWith("/api/spreadsheets/writeback/audits?status=failed&limit=50");
    expect(fetchMock).toHaveBeenCalledWith("/api/spreadsheets/writeback/audits?status=failed&limit=50&offset=1");
    expect(screen.getByText("job-failed-writeback-1：failed｜row 7｜permission denied")).toBeInTheDocument();
    expect(screen.getByText("job-failed-writeback-2：failed｜row 8｜sheet header not found")).toBeInTheDocument();
  });
  ...
  it("loads skipped spreadsheet writeback audits for drilldown", async () => {
    ...
    expect(await screen.findByText("Writeback audits total：1")).toBeInTheDocument();
    expect(
      screen.getByText("job-skipped-writeback-1：skipped｜row 无｜spreadsheet row mapping not found")
    ).toBeInTheDocument();
  });
